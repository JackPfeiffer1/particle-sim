...
TOTAL_BALLS = 500
GRAVITY = 1500
REPULSION_RADIUS = BALL_RADIUS * 3
ATTRACTION_RADIUS = BALL_RADIUS * 8
...
class Ball:
    def __init__(self, x,y, color):
        ...
        self.color = color # color changes based on height for visual interest
        
    def update(self, dt):
        # ocean gradient color
        top_color = (50, 130, 255)   # bright blue
        bottom_color = (5, 10, 25)   # almost black
        ...
        if t < 3/4:
            ...
            t = (t - 3/4) * 4   # remap bottom third to 0→1

        r = int(top_color[0] * (1 - t) + bottom_color[0] * t)
        g = int(top_color[1] * (1 - t) + bottom_color[1] * t)
        b = int(top_color[2] * (1 - t) + bottom_color[2] * t)

        self.color = (r, g, b)# update color based on height
        self.vy += GRAVITY * dt # gravity increases downward velocity
...
def compute_force(dist):
    if dist < REPULSION_RADIUS:
        ...
        return -(REPULSION_STRENGTH * (1 - dist / REPULSION_RADIUS)) # stronger near 0 
    ...
    for i, ball in enumerate(balls):
        ...
        for dx, dy in NEIGHBOR_OFFSETS:
                ...
                for j, other in grid.get(neighbor_cell, []):
                    ...
                    # VISCOSITY
                    rel_vx = other.vx - ball.vx
                    rel_vy = other.vy - ball.vy
                    viscosity = 0.05
                    ball.ax += viscosity * rel_vx
                    ball.ay += viscosity * rel_vy
                    other.ax -= viscosity * rel_vx
                    other.ay -= viscosity * rel_vy
...
def mouseForce(balls, mouse_pos, dt):
    mx, my = mouse_pos
    for ball in balls:
        delta_x = mx - ball.x
        delta_y = my - ball.y
...
for i in range(TOTAL_BALLS):
        ...
        color = (int(r*25), int(g*25), int(b*255))
...
while running:
    ...
    if pygame.mouse.get_pressed()[0]:
        mouseForce(balls, pygame.mouse.get_pos(), frame_dt)
    if pygame.mouse.get_pressed()[2]:
        balls.append(Ball(pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1], (255,255,255)))
    ...
    pygame.display.set_caption(f"FPS: {clock.get_fps():.0f} - Balls: {len(balls)}")
    pygame.display.flip()
